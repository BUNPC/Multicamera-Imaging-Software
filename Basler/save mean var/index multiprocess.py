...
def obtain(q_imgs_process,camera,num_frame):
    q_size = len(q_imgs_process)
    starth = time.time()
    ...
    while total_frame_ind < num_frame:
        ...
        if total_frame_ind % 100 == 0:
            print('Acquiring frame # ' + str(total_frame_ind) + ', ' + str(datetime.now()) + ', q = ' + str(q_imgs_process[0].qsize()) + "\n")
        ...
        q_imgs_process[q_ind].put(img)

    endh = time.time()
    print('Obtained all frames in ' + str(endh-starth) + " seconds.\n")

def analysis_process(q_imgs_process,camera_ind,num_frame):
    print('Start analysis process')
    total_frame_ind = 0
    ...
    pool.submit(save_h5,q_analyzed,camera_ind,num_frame,num_frame_per_file,save_folder,bit_depth,image_y,image_x)

    # wait for all tasks to complete
    pool.shutdown(wait=True)

    # get the images from separate process and send them over to different image queues
    while total_frame_ind < num_frame:
        q_ind = total_frame_ind % num_analysis_threads
        img = q_imgs_process.get()
...
def save_h5(q_analyzed,camera_ind,num_frame,num_frame_per_file,save_folder,bit_depth,image_y,image_x):
    print('Save started')
...
def acquire(camera_ind,use_trigger,bit_depth,gain,black_level,exp_time,frame_rate,image_y,image_x,num_frame,num_frame_per_file,save_folder):
    ...
    # mp queue for acquisition process to send data to 
    q_imgs_process = mp.Queue()


    # Add some tasks to the pool
    p1 = mp.Process(target=obtain, args=(q_imgs_process,camera,num_frame))
    p2 = mp.Process(target=analysis_process, args=(q_imgs_process,camera_ind,num_frame))
...
if __name__ == "__main__":
    ...
    save_folders = ['C:\\temp\\20231003\\10bit16db25blacklevel1000uscapon_windowed']
